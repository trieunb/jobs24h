					...
					<table class="table table-striped table-hover table-bordered">
								...
								<tbody>
									<tr>
										...
										<td>
											<strong class="text-blue"><em>{{$job->vitri}}</em></strong>
											<small><div class="legend text-orange">{{$status_job}}</div></small>
											<button type="button" class="btn bg-gray-light btn-sm">Thêm ghi chú</button>
										</td>
										<td>{{$job->ntd->company->company_name}}</td>
										<td>{{$my_job->save_date}}</td>
										<td>{{$my_job->respond}}</td>
										<td>
											@if(count($my_job->application->job_id) > 0)
												Đã ứng tuyển<br>({{date('d-m-Y',strtotime($my_job->application->apply_date))}})
											@else
												{{Form::button('Ứng tuyển', array('class'=>'btn btn-lg btn-default btn-apply'))}}
											@endif
										</td>
									</tr>
								</tbody>
