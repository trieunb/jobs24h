  			...
  			<ul>
  				@if(count($folders))
					@foreach($folders as $value)
						<li class="{{ HTML::active(['employers.candidates.folder'], 'selected', $value->id) }}"><a href="{{ URL::route('employers.candidates.folder', $value->id) }}"><i class="fa fa-folder-o"></i>{{ $value->folder_name }} ({{ $value->application->count() }})</a></li>
					@endforeach
  				@endif
