@extends('layouts.employer')
@section('title') Đổi email truy cập @stop
@section('content')
<section class="main-content">
		<div class="panel panel-default panel-sm">
			<div class="panel-heading">
			<div class="heading-form">
				<h2>Đổi email truy cập</h2>
				<span class="pull-right">(<span class="text-red">*</span>) Thông tin bắt buộc</span>
			</div>
			</div>
			<div class="panel-body">
				{{ Form::open(['route'=>'employers.account.changeemail', 'method'=>'POST', 'class'=>'form-horizontal']) }}
					@include('includes.notifications')
					<div class="form-group">
						<label for="input" class="col-sm-4 control-label">Email truy cập hiện tại:</label>
						<div class="col-sm-7">
							<label class="control-label">{{ $user->email }}</label>
						</div>
					</div>
					<div class="form-group">
						<label for="input" class="col-sm-4 control-label"><abbr>*</abbr>Xác nhận mật khẩu:</label>
						<div class="col-sm-7">
							{{ Form::password('password', ['required', 'class'=>'form-control']) }}
						</div>
					</div>
					<div class="form-group">
						<label for="input" class="col-sm-4 control-label"><abbr>*</abbr>Nhập email truy cập mới:</label>
						<div class="col-sm-7">
							{{ Form::email('email', null, ['required', 'class'=>'form-control']) }}
						</div>
						<div class="col-sm-1">
							<abbr class="question" data-toggle="tooltip" data-placement="bottom" title="Tooltip on bottom"><i class="fa fa-question "></i></abbr>
						</div>
					</div>
					<div class="form-group">
						<label for="input" class="col-sm-4 control-label"><abbr>*</abbr>Xác nhận email truy cập mới:</label>
						<div class="col-sm-7">
							{{ Form::email('email_confirmation', null,  ['required', 'class'=>'form-control']) }}
						</div>
					</div>
					<div class="form-group">
						<div class="col-sm-offset-4 col-sm-7">
							<button type="submit" class="btn btn-lg bg-blue">LƯU</button>
							<button type="button" id="btn-huy" class="btn btn-lg bg-blue">HỦY</button>
						</div>
					</div>
				</form>
			</div>
		</div>
	</section>
	@stop

	@section('script')
	<script type="text/javascript">
	$('#btn-huy').click(function(event) {
		$('input').each(function(index, el) {
			$(this).val('');
		});
	});
	</script>
@stop