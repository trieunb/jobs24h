					...
					<table class="table table-bordered table-blue-bordered white rs-table">
						...
						<tbody>
							...
							<tr>
								...
								<td>
									<a href="{{ URL::route('employers.search.resumeinfo', $ap->cv_id) }}" class="text-blue decoration"><strong>{{ $ap->ntv->full_name() }}</strong></a>
								...
								<td>
									<a href="{{ URL::route('employers.search.resumeinfo', $ap->cv_id) }}" class="text-blue decoration"><strong>{{ $ap->full_name() }}</strong></a>
									<span class="fa fa-paperclip"></span>
									<br>
									Chức danh: {{ $ap->headline }}<br>
									Địa Điểm: {{ $ap->address }}
									<br>
									Thư mục: {{ HTML::link(URL::route('employers.candidates.job', $ap->job_id), $ap->job->vitri) }}
								</td>
								...
								<td>
									<div class="dropdown">
									  ...
									  <ul class="dropdown-menu" aria-labelledby="dropdown{{ $ap->id }}">
									    ...
									    <li><a id="st_{{$ap->id}}" class="modalSaveStatus">Cập nhật trạng thái</a></li>
									    <!--<li><a id="lt_{{$ap->id}}" class="modalSendLetter">Gửi thông báo</a></li>-->
									    <li><a href="{{ URL::route('employers.search.print_cv', $ap->cv_id) }}" target="_blank">In</a></li>
									  </ul>
									</div>
								</td>
								@endif
...
@section('style')
	<style type="text/css">
		...
		.table-blue-bordered tr td {
			line-height: 20px;
		}
		.table-blue-bordered tr td a {
			color: #00b9f2;
		}
		.table-blue-bordered tr td a:hover {
			color: #F7941D;
		}
	</style>
@stop
